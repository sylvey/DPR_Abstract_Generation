...
class Fragments(object):
    ...
    def novelty(self, n=2):
        """
        Calculate N-gram Novelty.
        Metric: Proportion of n-grams in summary that are NOT in the source text.
        High Novelty = High Abstractiveness (Rewriting)
        Low Novelty  = High Extractiveness (Copying)
        
        Arguments:
            - n (int): n-gram order (default 2 for Bigram Novelty)
        """
        summary_ngrams = self._get_ngrams(self._norm_summary, n)
        text_ngrams = self._get_ngrams(self._norm_text, n)
        
        if not summary_ngrams:
            return 0.0
            
        # Calculate set difference: n-grams in summary BUT NOT in text
        novel_ngrams = summary_ngrams - text_ngrams
...
def evaluate_batch(summaries, sources):
    ...
    for i, (summ, src) in enumerate(zip(summaries, sources)):
        try:
            frag = Fragments(summ, src)
            
            metrics["coverage"].append(frag.coverage())
            metrics["density"].append(frag.density())
            metrics["compression"].append(frag.compression())
            metrics["novelty2"].append(frag.novelty(n=2))
            metrics["novelty3"].append(frag.novelty(n=3))

        except Exception as e:
            print(f"Error in  {i}: {e}")
            metrics["coverage"].append(0.0)
            metrics["density"].append(0.0)
            metrics["compression"].append(0.0)
            metrics["novelty2"].append(0.0)
            metrics["novelty3"].append(0.0)

    results = {
        "avg_coverage": np.mean(metrics["coverage"]),
        "avg_density": np.mean(metrics["density"]),
        "avg_compression": np.mean(metrics["compression"]),
        "avg_novelty2": np.mean(metrics["novelty2"]),
        "avg_novelty3": np.mean(metrics["novelty3"])
    }
...
def main():
    ...
    parser.add_argument("--id_type", default="article_id", help="Input JSONL file")
    parser.add_argument("--pred", default="pred", help="Input JSONL file")
    # parser.add_argument("--mapping_file", default="mappings.pkl", help="Input JSONL file")
    ...
    if args.id_type == 'abstract' or args.id_type == "reference":
        ...
        with open("mappings.pkl", 'rb') as f:
            ...
            if args.id_type == 'id':
                id_dict = loaded_index_dict
            elif args.id_type == 'article_id':
                id_dict = loaded_article_dict
    ...
    with open(args.pairs, "r", encoding="utf-8") as f:
        for line in f:
            ...
            refs.append(str(id_dict[record[args.id_type]]).rstrip("\n"))
            count += 1
    

    results = evaluate_batch(preds, refs)
    print(f"[Evaluated] Successfully evaluated {len(preds)} pairs from {args.pairs}")
    print(f"Average Coverage: {results['avg_coverage']:.4f}")
    print(f"Average Density: {results['avg_density']:.4f}")
    print(f"Average Compression: {results['avg_compression']:.4f}")
    print(f"Average Novelty-2: {results['avg_novelty2']:.4f}")
    print(f"Average Novelty-3: {results['avg_novelty3']:.4f}")
